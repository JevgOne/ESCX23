...
### Inline keyboard (tlacitka jako pomocnici)

AI odpovedi mohou obsahovat inline keyboard tlacitka jako **navodne pomucky**.
Klient muze vzdy psat volne — tlacitka jsou jen shortcut.

```typescript
// Handler po kazde AI odpovedi analyzuje text a prida relevantni tlacitka:

// Pokud AI zminuje divky → tlacitka s jejich jmeny
// "Dnes pracuji Emily, Nika a Caty" → [Emily] [Nika] [Caty]

// Pokud AI zminuje casy → tlacitka s casy
// "Emily ma volno v 15:30, 16:00 a 17:00" → [15:30] [16:00] [17:00]

// Pokud AI zminuje delky → tlacitka s delkami
// "Na jak dlouho?" → [30 min] [45 min] [60 min] [90 min] [120 min]

// Pokud AI se pta na potvrzeni → [Potvrdit] [Zrusit]

// Implementace: AI vraci structured output s optional keyboard hints,
// nebo handler parsuje text a generuje tlacitka automaticky.
```

### Callback query handling

```typescript
// Kdyz klient klikne na tlacitko → callback_query update
// Handler prevede callback_data na text a posle AI jako beznou zpravu:
//   callback "girl:Emily" → "Emily"
//   callback "time:16:00" → "16:00"
//   callback "dur:60" → "60 min"
//   callback "confirm" → "Potvrdit"
//   callback "cancel" → "Zrusit"
// AI pak zpracuje jako kdyby klient napsal text rucne.
// → Jednotny flow pro text i tlacitka.
```
