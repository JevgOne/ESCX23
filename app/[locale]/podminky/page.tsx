...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'podminky' });
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const title = `${t('h1')} — ${tCommon('siteName')}`;
  const canonical = getCanonicalUrl(locale, '/podminky');
  const ogImages = await buildOgImages('podminky', locale, '/podminky', title);
...
export default async function PodminkyPage({ params }: Props) {
  const { locale } = await params;
  ...
  const breadcrumbSchema = breadcrumbListJsonLd([
    { name: tNav('terms'), url: getCanonicalUrl(locale, '/podminky') },
  ]);
  ...
  return (
    <main>
      ...
      />
      <Breadcrumbs items={[{ label: tNav('terms') }]} locale={locale} />
