...
/**
 * Cron (Sunday 16:00 UTC = 18:00 Prague) — remind girls who haven't submitted
 * at least 2 shifts for the upcoming week.
 *
 * Creates a notification in the system that the Studio PWA can display.
 */
export async function GET(request: Request) {
  ...
  // Create reminder notifications for girls via their Studio (stored in admin_notifications for now)
  if (missing.length > 0) {
    await db.execute({
      ...
      args: [
        'shift_reminder',
        'Připomínka směn',
        `${missing.length} dívek ještě nezadalo dostatek směn na příští týden: ${missing.join(', ')}`,
