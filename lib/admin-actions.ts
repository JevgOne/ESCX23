...
export async function adminUploadVoice(formData: FormData) {
  ...
  if (deleteOnly) {
    ...
    return adminRedirect(`/admin/divky/${girlId}/edit`);
  }

  const file = formData.get('voice') as File | null;
  if (!file || file.size === 0) {
    return adminRedirect(`/admin/divky/${girlId}/edit?error=${encodeURIComponent('Nebyl vybrán žádný soubor')}`);
  }

  const ext = (file.name.split('.').pop() ?? '').toLowerCase();
  const allowed = new Set(['mp3', 'wav', 'ogg', 'webm', 'm4a', 'aac']);
  if (!allowed.has(ext)) {
    return adminRedirect(`/admin/divky/${girlId}/edit?error=${encodeURIComponent('Nepodporovaný formát. Povol: MP3, WAV, OGG, M4A.')}`);
  }

  if (file.size > 5 * 1024 * 1024) {
    return adminRedirect(`/admin/divky/${girlId}/edit?error=${encodeURIComponent('Soubor je příliš velký (max 5 MB).')}`);
  }
...
export async function addGirlSchedule(formData: FormData) {
  ...
  // Apartment picked once for the whole submit; each day may override it below.
  ...
  for (let i = 0; i <= 6; i++) {
    ...
    // Apartment belongs to the day. It used to be read once for the whole
    // submit and stamped onto every ticked day, so a girl working Tuesday in
    // Žižkov and Thursday in Nové Město could not be entered at all — and with
    // no edit action, a wrong apartment meant deleting the day and starting over.
